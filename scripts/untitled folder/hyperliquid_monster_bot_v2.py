"""
Hyperliquid Monster Bot v2.3 - OPTIMIZED Multi-Strategy Perpetual Trading

UPGRADES from v2.2:
- Fixed Hyperliquid symbol names: kPEPE, kBONK (1000x multiplier tokens)
- Removed unavailable coins: MOG, JEFF (not on Hyperliquid)
- Fixed minimum order size: $10 minimum (Hyperliquid requirement)
- 14 validated pairs for funding scanning
...
Date: 2026-01-12
Version: 2.3 (Fixed Hyperliquid symbols)
"""
...
class HyperliquidMonsterBotV2(ScriptStrategyBase):
    ...
    # All validated Hyperliquid pairs (kPEPE/kBONK use 1000x format)
    # PURR removed - 55.8% volatility exceeds safe thresholds
    ...
    def __init__(self, connectors: Dict[str, ConnectorBase], config: HyperliquidMonsterV2Config = None):
        ...
        self._first_run = True  # Flag to run activation on first tick
    ...
    def on_start(self):
        ...
        # Shutdown flag to prevent new orders during stop
    ...
    def _activate_strategies(self):
        """Activate strategies on first tick (fallback if on_start not called)."""
        self.logger().info("=" * 50)
        ...
        if self.config.funding_enabled:
            self.metrics["funding"].status = StrategyMode.ACTIVE
            self.logger().info("FUNDING: Strategy activated")
        if self.config.grid_enabled:
            self.metrics["grid"].status = StrategyMode.ACTIVE
            self.logger().info("GRID: Strategy activated")
        if self.config.momentum_enabled:
            self.metrics["momentum"].status = StrategyMode.WARMING_UP
            self.logger().info("MOMENTUM: Strategy warming up")
    ...
    def on_tick(self):
        ...
        # Prevent new orders during shutdown
        if getattr(self, '_is_stopping', False):
            return

        # First run activation (since on_start() isn't reliably called)
        if self._first_run:
            self._first_run = False
            self._activate_strategies()
    ...
    def on_stop(self):
        ...
        # Set stopping flag FIRST to prevent on_tick from placing new orders
        self._is_stopping = True

        # Cancel all orders BEFORE closing positions to prevent race conditions
        self._cancel_all_orders()
        self.logger().info("All orders cancelled.")

        # Now close positions (this won't trigger new orders since _is_stopping is True)
    ...
    def _scan_funding_opportunities(self):
        ...
        for pair in self.config.funding_scan_pairs.split(","):
            ...
            try:
                ...
                time_to_funding = next_funding_ts - self.current_timestamp
        ...
        elif self.funding_opportunities:
    ...
    def _manage_funding_positions(self):
        ...
        for pair, current_opp in list(self.funding_positions.items()):
            try:
                ...
                time_to_funding = float(funding_info.next_funding_utc_timestamp) - self.current_timestamp
                minutes_to_funding = time_to_funding / 60
    ...
    def _initialize_grid(self, base_price: Decimal):
        ...
        pair = self.config.grid_pair
        existing_orders = [o for o in self.get_active_orders(connector_name=self.config.exchange)
                          if o.trading_pair == pair]
        if existing_orders:
    ...
    def _place_perpetual_order(
        ...
        self.logger().debug(f"[{strategy}] Placing {side.name} order for {pair} @ {price} ({leverage}x)")
        try:
    ...
    def format_status(self) -> str:
        ...
        lines.append(f"  HIGH (3x): DOGE, HYPE | EXTREME (2x): AVNT, kPEPE, kBONK, WIF, VVV, HYPER, IP")
