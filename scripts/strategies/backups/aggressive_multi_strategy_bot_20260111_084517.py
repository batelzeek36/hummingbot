"""
Aggressive Multi-Strategy Trading Bot (Paper Trading Version)

This is the AGGRESSIVE version designed for paper trading testing.
Combines multiple strategies with tighter parameters for maximum activity:
- Aggressive Market Making (tight spreads, fast refresh)
- Dynamic Grid Trading (more levels, adaptive spacing)
- RSI Directional with momentum confirmation
...
class AggressiveConfig(BaseClientModel):
    ...
    exchange: str = Field(
        ...
        description="Exchange to trade on (paper trading)"
    )
...
class AggressiveMultiStrategyBot(ScriptStrategyBase):
    ...
    def _run_rsi_strategy(self, pair: str):
        ...
        if position is None:
            ...
            if rsi < self.config.rsi_oversold:
                ...
                # Overbought - go short (or skip in spot)
                pass  # Skip shorts for spot trading
    ...
    def _exit_rsi_position(self, pair: str, price: Decimal, reason: str):
        ...
        amount = connector.get_balance(base)
        if amount <= 0:
            self.rsi_position[pair] = None
            return

        # Sell portion used for RSI
        capital = self.pair_capital.get(pair, Decimal("0"))
        order_amount_usd = capital * self.config.rsi_order_amount_pct / 100
        order_amount = min(order_amount_usd / price, amount)
        ...
        if entry_price:
            ...
            self.logger().info(f"RSI EXIT {pair} at {price} ({reason}), PnL: ${pnl:.2f}")
    ...
    def _exit_macd_position(self, pair: str, price: Decimal, reason: str):
        ...
        if entry_price:
            ...
            self.logger().info(f"MACD EXIT {pair} at {price} ({reason}), PnL: ${pnl:.2f}")
    ...
    def _exit_momentum_position(self, pair: str, price: Decimal, reason: str):
        ...
        if entry_price:
            ...
            self.logger().info(f"MOMENTUM EXIT {pair} at {price} ({reason}), PnL: ${pnl:.2f}")
