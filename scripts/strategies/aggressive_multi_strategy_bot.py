"""
Aggressive Multi-Strategy Trading Bot (Paper Trading Version with SHORTING)

This is the AGGRESSIVE version designed for paper trading testing.
Uses Binance Perpetual Testnet for LONG and SHORT capabilities.

Strategies with SHORT support:
- Aggressive Market Making (tight spreads, fast refresh) - LONG only
- Dynamic Grid Trading (more levels, adaptive spacing) - LONG only
- RSI Directional (LONG on oversold, SHORT on overbought)
- MACD Trend Following (LONG on bullish crossover, SHORT on bearish crossover)
- Momentum Scalping (LONG on upward spikes, SHORT on downward spikes)
- Multi-coin rotation based on volatility

WARNING: This is for PAPER TRADING ONLY.
Do NOT run on live funds without extensive testing and parameter adjustment.

Usage:
    start --script aggressive_multi_strategy_bot.py
...
class AggressiveConfig(BaseClientModel):
    """Configuration for Aggressive Multi-Strategy Bot."""

    script_file_name: str = Field(default="aggressive_multi_strategy_bot.py")

    # === EXCHANGE SETTINGS ===
    exchange: str = Field(
        default="binance_perpetual_testnet",
        description="Exchange to trade on (Binance Perpetual Testnet - paper money with shorting)"
    )

    # === TRADING PAIRS (Optimized for each strategy via ccxt analysis) ===
    # BTC-USDT: Best for Market Making (highest liquidity, tightest spread 0.0001%)
    # LTC-USDT: Best for Grid Trading (high oscillation 63.0, moderate volatility 0.75%)
    # RENDER-USDT: Best for RSI/MACD (strong trend +8.39, RSI 72.3 overbought signal)
    # SUI-USDT: Strong for MACD Trend (trend strength 3.55)
    # PEPE-USDT: Best for Momentum Scalping (high volatility 2.29%, RSI 27.9 oversold)
    trading_pairs: str = Field(
        default="BTC-USDT,LTC-USDT,RENDER-USDT,SUI-USDT,PEPE-USDT",
        description="Comma-separated list of trading pairs (optimized per strategy)"
    )

    # === CAPITAL SETTINGS ===
    total_capital: Decimal = Field(
        default=Decimal("100000"),
        description="Total capital to allocate (paper trading)"
    )

    reserve_pct: Decimal = Field(
        default=Decimal("10"),
        description="Percentage to keep as reserve"
    )
    ...
    # === AGGRESSIVE RSI SETTINGS (with SHORT capability) ===
    rsi_enabled: bool = Field(default=True)
    rsi_enable_shorts: bool = Field(default=True, description="Enable SHORT on overbought signals")
    rsi_period: int = Field(default=10)  # Faster RSI
    rsi_oversold: int = Field(default=25)  # More aggressive entry
    rsi_overbought: int = Field(default=75)
    rsi_order_amount_pct: Decimal = Field(default=Decimal("5"))  # 5% of capital
    rsi_take_profit_pct: Decimal = Field(default=Decimal("0.015"))  # 1.5% take profit
    rsi_stop_loss_pct: Decimal = Field(default=Decimal("0.01"))  # 1% stop loss
    rsi_cooldown_minutes: int = Field(default=15)  # Shorter cooldown

    # === MACD TREND FOLLOWING (with SHORT capability) ===
    macd_enabled: bool = Field(default=True)
    macd_enable_shorts: bool = Field(default=True, description="Enable SHORT on bearish crossovers")
    ...
    macd_take_profit_pct: Decimal = Field(default=Decimal("0.02"))  # 2%
    macd_stop_loss_pct: Decimal = Field(default=Decimal("0.01"))  # 1%
    macd_cooldown_minutes: int = Field(default=30)

    # === MOMENTUM SCALPING (with SHORT capability) ===
    momentum_enabled: bool = Field(default=True)
    momentum_enable_shorts: bool = Field(default=True, description="Enable SHORT on downward momentum")
    ...
    momentum_take_profit_pct: Decimal = Field(default=Decimal("0.008"))  # 0.8%
    momentum_stop_loss_pct: Decimal = Field(default=Decimal("0.005"))  # 0.5%
    ...
    mm_max_loss: Decimal = Field(default=Decimal("2000"))
    grid_max_loss: Decimal = Field(default=Decimal("3000"))
    rsi_max_loss: Decimal = Field(default=Decimal("2500"))
    macd_max_loss: Decimal = Field(default=Decimal("2500"))
    momentum_max_loss: Decimal = Field(default=Decimal("1500"))
    global_max_loss: Decimal = Field(default=Decimal("10000"))
    global_max_drawdown_pct: Decimal = Field(default=Decimal("10"))
    daily_loss_limit: Decimal = Field(default=Decimal("5000"))
...
class AggressiveMultiStrategyBot(ScriptStrategyBase):
    """
    Aggressive multi-strategy trading bot for paper trading.
    ...
    3. RSI Directional - Faster signals, tighter stops
    4. MACD Trend Following - Capture medium-term trends
    5. Momentum Scalping - Quick trades on price spikes
    6. Volatility Rotation - Allocate more to volatile coins
    """

    @classmethod
    def init_markets(cls, config: AggressiveConfig):
    ...
    # Optimized pairs: MM=BTC, Grid=LTC, RSI/MACD=RENDER, MACD=SUI, Momentum=PEPE
    markets = {"binance_perpetual_testnet": {
        "BTC-USDT", "LTC-USDT", "RENDER-USDT", "SUI-USDT", "PEPE-USDT"
    }}

    def __init__(self, connectors: Dict[str, ConnectorBase], config: AggressiveConfig = None):
        super().__init__(connectors)
        self.config = config if config is not None else AggressiveConfig()
    ...
    def _allocate_capital(self):
        """Allocate capital across trading pairs."""
    ...
    def on_start(self):
        ...
        self.logger().info("=" * 60)
        self.logger().info("AGGRESSIVE MULTI-STRATEGY BOT STARTING")
        self.logger().info(f"Exchange: {self.config.exchange}")
        self.logger().info(f"Trading Pairs: {self.trading_pairs}")
        self.logger().info(f"Total Capital: ${self.config.total_capital}")
    ...
    def _run_rsi_strategy(self, pair: str):
        ...
        if position is None:
            ...
            if rsi < self.config.rsi_oversold:
                ...
                # Overbought - go short (perpetual futures)
            ...
            if entry_price:
                if position == "long":
                    ...
                    # RSI reversal (exit long when RSI rises)
                    elif rsi > 60:
                    ...
                    # Stop loss
                        ...
                        self._exit_rsi_position(pair, mid_price, "stop_loss")
                    # RSI reversal (exit short when RSI drops)
                    elif rsi < 40:
                        self._exit_rsi_position(pair, mid_price, "rsi_reversal")
    ...
    def _enter_rsi_position(self, pair: str, direction: str, price: Decimal):
        """Enter RSI position (long or short)."""
        connector = self.connectors[self.config.exchange]
        ...
        if direction == "long":
            ...
            self.logger().info(f"RSI LONG {pair} at {price}")

        elif direction == "short":
            # For perpetual futures: SELL to open short position
            order_id = connector.sell(
                trading_pair=pair,
                amount=order_amount,
                order_type=OrderType.MARKET,
            ...
            self.rsi_entry_price[pair] = price
            self.rsi_last_signal_time[pair] = datetime.now()
            self.logger().info(f"RSI SHORT {pair} at {price}")
    ...
    def _exit_rsi_position(self, pair: str, price: Decimal, reason: str):
        ...
        if position == "long":
            ...
            if entry_price:
                # For shorts, profit = (entry - exit) * amount
    ...
    def _run_macd_strategy(self, pair: str):
        ...
        if position is None:
            ...
            # Bearish crossover - go short (if enabled)
    ...
    def _enter_macd_position(self, pair: str, direction: str, price: Decimal):
        """Enter MACD position (long or short)."""
        connector = self.connectors[self.config.exchange]
        ...
        if direction == "long":
            ...
            self.logger().info(f"MACD LONG {pair} at {price}")
            ...
            self.macd_entry_price[pair] = price
            self.macd_last_signal_time[pair] = datetime.now()
            self.logger().info(f"MACD SHORT {pair} at {price}")
    ...
    def _run_momentum_strategy(self, pair: str):
        ...
        if position is None:
            ...
            # Strong downward momentum - enter short (if enabled)
            ...
            entry_time = self.momentum_entry_time.get(pair, datetime.now())
            ...
            if entry_price:
                if position == "long":
                    ...
                    elif (datetime.now() - entry_time).total_seconds() > 300:
    ...
    def _enter_momentum_position(self, pair: str, direction: str, price: Decimal):
        """Enter momentum position (long or short)."""
        connector = self.connectors[self.config.exchange]
        ...
        if direction == "long":
            ...
            self.logger().info(f"MOMENTUM LONG {pair} at {price}")
            ...
            self.momentum_entry_price[pair] = price
            self.momentum_entry_time[pair] = datetime.now()
            self.logger().info(f"MOMENTUM SHORT {pair} at {price}")
    ...
    def format_status(self) -> str:
        ...
        lines.append("")
        lines.append("=" * 60)
        lines.append("  AGGRESSIVE MULTI-STRATEGY BOT STATUS")
        lines.append("=" * 60)
